...
function ProjectDetail() {
  ...
  const getProjectInfo = async (projectName) => {
    let sqlstr=`select k.screen_name,k.totalScore,k.updatedTime,x.followers,x.description from kolXAccountDetail k inner join XAccounts x on k.screen_name=x.screen_name where k.screen_name='${projectName}'`;
  ...
  return (
    <div style={{ maxWidth: '640px', margin: '0 auto',padding:'10px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', border: '1px solid #ddd', borderRadius: '10px', padding: '15px' }}>
        <div style={{ flex: 1 }}>
          <p>KOL名称：{projectInfo.name}</p>
          <p>粉丝数 ：{projectInfo.followers}</p>
          <p>评分   ：{projectInfo.score}</p>
        </div>
        <div style={{ flex: 1 }}>
      ...
      <div style={{ marginTop: '20px', border: '1px solid #ddd', borderRadius: '10px', padding: '15px' }}>
        ...
        <div style={{ textAlign: 'center' }}>
          <button onClick={handleCommentSubmit} style={{ padding: '5px 20px' }}>发表</button>
        </div>
      </div>
      {/* 这里可以添加更多项目详情内容 */}
    </div>
